    ...
    public class InstallerWindow : EditorWindow
    {
        ...
        private void InstallScreen()
        {
            ...
            //using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                using (new EditorGUILayout.HorizontalScope())
                {
                    using (new EditorGUI.DisabledGroupScope(Installer.Demo.HAS_PACKAGE == false))
                    {
                    ...
                    if (!Installer.Demo.INSTALLED)
                    {
                        if (Installer.Demo.HAS_PACKAGE == false) SCPE_GUI.DrawStatusBox(null, "Missing", SCPE_GUI.Status.Warning, false);
                    }

                }
                using (new EditorGUILayout.HorizontalScope())
                {
                    if (Installer.Demo.HAS_PACKAGE == true)
                    {
                        EditorGUILayout.HelpBox("Demo scenes, profile and sample textures", MessageType.None);
                    }
                    if (Installer.Demo.HAS_PACKAGE == false && Installer.Demo.INSTALLED == false)
                    {
                        EditorGUILayout.HelpBox("Also import the \"_DemoContents.unitypackage\" file to install demo content.", MessageType.None);
            ...
            if (Installer.CURRENTLY_INSTALLING || Installer.IS_INSTALLED)
            {
                ...
                EditorGUILayout.Space();
                using (new EditorGUILayout.VerticalScope(EditorStyles.textArea, UnityEngine.GUILayout.MaxHeight(175f)))
                {
        ...
        private void FinalScreen()
        {
            ...
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                if (Installer.Settings.installDemoContent == false && Installer.Demo.HAS_PACKAGE == true)
                {
